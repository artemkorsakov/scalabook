...
trait PlusLaw extends SemigroupLaw:
  def checkPlusLaw[F[_]: Plus, A](f1: F[A], f2: F[A], f3: F[A]): Unit =
    assertEquals(
      Plus[F].plus(Plus[F].plus(f1, f2), f3),
      Plus[F].plus(f1, Plus[F].plus(f2, f3)),
      "Associativity"
    )
    checkSemigroupLaw(f1, f2, f3)(using Plus[F].semigroup)

  def checkPlusLawWithRunner[F[_]: Plus: Runner1, A](
      f1: F[A],
      f2: F[A],
      f3: F[A]
  ): Unit =
  ...
  def checkPlusLawWithRunner[F[_]: Plus, A](f1: F[A], f2: F[A], f3: F[A])(
      run: F[A] => A
  ): Unit =
    assertEquals(
