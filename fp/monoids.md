...
`(M, +)` является [моноидом](@TC@monoid/monoid) для заданного множества `M` и операции `+`,
...
Она берет начальный элемент и объединяет его с типом [_Foldable_](@TC@monad/foldable), 
