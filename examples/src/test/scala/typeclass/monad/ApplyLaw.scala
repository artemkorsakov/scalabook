...
trait ApplyLaw extends FunctorLaw, SemigroupalLaw:
  def checkApplyLaw[F[_]: Apply, A, B, C](fa: F[A], fab: F[A => B], fbc: F[B => C])(using
    ...
    assertEquals(
      ...
      Apply[F].apply(Apply[F].apply(fbc.map((bc: B => C) => (ab: A => B) => bc compose ab))(fab))(fa),
      "composition"
    )
  ...
  def checkApplyLawWithRunner[F[_]: Apply: Runner1, A, B, C](fa: F[A], fab: F[A => B], fbc: F[B => C])(using
    ...
    assertEquals(
      ...
      Runner1[F].run(Apply[F].apply(Apply[F].apply(fbc.map((bc: B => C) => (ab: A => B) => bc compose ab))(fab))(fa)),
      "composition"
    )
