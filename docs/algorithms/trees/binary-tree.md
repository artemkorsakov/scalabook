...
- путь (_path_): определяется как последовательность узлов (`x[0], x[1], x[2], ..., x[n]`), 
...
- корневой путь (_root path_): для узла `x[0]` его корневой путь определяется как путь (`x[0], x[1], x[2], ..., x[n]`), где `x[n]` — корень дерева. 
