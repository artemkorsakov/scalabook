...
потому что единичный элемент не удовлетворяет закону "тождественности" (`0 * x != x` для всех `x != 0`).
...
- формулирование четких законов (хотя бы на бумаге) структуры помогает избегать "самозванцев",
