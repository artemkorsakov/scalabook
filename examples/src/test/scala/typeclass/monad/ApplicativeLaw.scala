...
trait ApplicativeLaw extends ApplyLaw:
  def checkApplicativeLaw[F[_]: Applicative, A, B, C](x: A)(using
    ...
    assertEquals(
      Applicative[F].apply[A, A](Applicative[F].unit(identity))(fa),
      fa,
      "identity"
    )
    assertEquals(
  ...
  def checkApplicativeLawWithRunner[F[_]: Applicative: Runner1, A, B, C](x: A)(
      using
      f: A => B,
      fReverse: B => A,
      g: B => C,
    ...
    val fa = Applicative[F].unit(x)
    checkApplyLawWithRunner[F, A, B, C](
      fa,
      Applicative[F].unit(f),
      Applicative[F].unit(g)
    )
