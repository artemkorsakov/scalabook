...
trait PlusEmptyLaw extends PlusLaw, MonoidLaw:
  def checkPlusEmptyLaw[F[_]: PlusEmpty, A](f1: F[A], f2: F[A], f3: F[A]): Unit =
    ...
    assertEquals(PlusEmpty[F].plus(f1, PlusEmpty[F].empty), f1, "right identity")
  ...
  def checkPlusEmptyLawWithRunner[F[_]: PlusEmpty, A](f1: F[A], f2: F[A], f3: F[A])(run: F[A] => A): Unit =
    ...
    assertEquals(run(PlusEmpty[F].plus(f1, PlusEmpty[F].empty)), run(f1), "right identity")
    assertEquals(run(PlusEmpty[F].plus(PlusEmpty[F].empty, f1)), run(f1), "left identity")
