...
trait ContravariantFunctorLaw extends InvariantFunctorLaw:
  def checkContravariantFunctorLaw[F[_]: ContravariantFunctor: Runner1, A, B, C](
    ...
    assertEquals(Runner1[F].run(ContravariantFunctor[F].cmap[A, A](fa)(x => x)), Runner1[F].run(fa), "identity")
    assertEquals(
      Runner1[F].run(ContravariantFunctor[F].cmap(ContravariantFunctor[F].cmap(fa)(fba))(fcb)),
