...
trait SemigroupalLaw extends Assertions:
  def checkSemigroupalLawWithRunner[F[_]: Semigroupal: Runner1, A, B, C](fa: F[A], fb: F[B], fc: F[C]): Unit =
    val product1: (A, (B, C)) =
      Runner1[F].run(Semigroupal[F].product[A, (B, C)](fa, Semigroupal[F].product[B, C](fb, fc)))
    val product2: ((A, B), C) =
      Runner1[F].run(Semigroupal[F].product[(A, B), C](Semigroupal[F].product[A, B](fa, fb), fc))
