...
Набор стандартных типов безусловно весьма ограничен и покрывает только самые распространенные ситуации.
Создатели языков программирования не могут, да и не должны предугадывать все типы,
которые будут использоваться в Вашей программе.
Список стандартных типов уже давно сформирован, пожалуй, самим временем 
и кочует из одного языка программирования в другой с незначительными изменениями.
...
Давайте, представим, что нам нужно создать переменную, которая представляла бы собой имя человека, 
...
Но все перечисленные невалидные варианты - это тип `String`.
Получается, что этот тип описывает нужное нам множество, но ещё множество невалидных вариантов,
...
И дальнейшее её использование в коде возможно только в качестве строки.

Уточняющий тип - это конкретный тип:
