...
trait DivisibleLaw extends DivideLaw:
  def checkDivideLaw[F[_]: Divisible: Runner1, A, B, C](
    ...
    assertEquals(Divisible[F].divide(fa1, Divisible[F].conquer)(delta), fa1, "rightIdentity")
    assertEquals(Divisible[F].divide(Divisible[F].conquer, fa1)(delta), fa1, "leftIdentity")
